...
from bootsoff.utils.topo import topo


class TopoTestCase(unittest.TestCase):
    def test_ddmm_to_dd(self):
        ddmm_angle = 6030.
        actual = topo.ddmm_to_dd(ddmm_angle)
        expected = 60.5
    ...
    def test_azimuth(self):
        ...
        actual = topo.azimuth(origin, target)
        expected = np.pi/6.
        self.assertAlmostEqual(actual, expected, places=12)
    ...
    def test_cclength2xz(self):
        ...
        actual = topo.cclength2xz(known_points, np.linspace(0, 800, 81))[65][0]
